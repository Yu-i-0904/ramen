...
$message_text = $json_object->{"events"}[0]->{"message"}->{"text"};    //メッセージタイプ
...
$message_address = $json_object->{"events"}[0]->{"message"}->{"address"};    //メッセージ内容
$total = $message_latitude + $message_longitude;

//メッセージタイプが「text」以外のときは何も返さず終了
if ($message_type != "location" || $message_text != "おすすめ") exit;
require('dbconnect.php');
$stmt = $dbh->query("select * from yamanote_lines order by abs(latitude + longitude - $total) limit 1");
$nearly_station = $stmt->fetch();
$nearly_station_id = $nearly_station['id'];

$stmt_2 = $dbh->query("select * from yamanote_lines inner join ramen_shops on yamanote_lines.id = ramen_shops.yamanote_line_id where yamanote_line_id = $nearly_station_id");
$shops = $stmt_2->fetchAll();
shuffle($shops);


//返信実行
sending_messages($accessToken, $replyToken, $name, $address, $latitude, $longitude, $shops);
...
//メッセージの送信
function sending_messages($accessToken, $replyToken, $name, $address, $latitude, $longitude, $shops) {
